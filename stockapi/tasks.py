...
@task(name="total-buysell-get-01")
def total_buysell_1():
    today = datetime.now().strftime('%Y%m%d')
    ticker = Ticker.objects.filter(date=today).order_by('id')
    ticker_count = ticker.count()
    ticker_cut = ticker_count//10
    ticker_list = ticker[:ticker_cut]
    buyselltotal(ticker_list)


## Task

def buysellrecent(ticker):
    success = False
    data_list=[]
    for i in range(len(ticker)):
...
@task(name="today-buysell-get-01")
def today_buysell_1():
    today = datetime.now().strftime('%Y%m%d')
    ticker = Ticker.objects.filter(date=today).order_by('id')
    ticker_count = ticker.count()
